    ...
    for num_samples in nums_samples:
        for model in models:
            for noise_color in noise_colors:
                fad_scores_config = []
                config = model + '_' + mode + '_' + str(num_samples) + '_' + noise_color
...
for mode in modes:
    ...
    for idx, model in enumerate(models):
        ...
        plt.title(model)
        plt.legend()
        plt.savefig('plots/' + mode + '_model_' + model + '.png')
        plt.show()
    ...
    for idx, num_samples in enumerate(nums_samples):
        plt.figure()
        for i in range(len(fad_scores)):
            if '_' + str(num_samples) + '_' in configs[i] and mode in configs[i]:
                plt.plot(snr_values, fad_scores[i], label=configs[i])
        plt.xlabel('SNR')
        plt.ylabel('FAD Score')
        plt.title(str(num_samples) + ' samples')
        plt.legend()
        plt.savefig('plots/' + mode + '_num_samples_' + str(num_samples) + '.png')
        plt.show()
    ...
    for idx, noise_color in enumerate(noise_colors):
        plt.figure()
        for i in range(len(fad_scores)):
            if noise_color in configs[i] and mode in configs[i]:
                plt.plot(snr_values, fad_scores[i], label=configs[i])
        plt.xlabel('SNR')
        plt.ylabel('FAD Score')
        plt.title(noise_color + ' noise')
        plt.legend()
        plt.savefig('plots/' + mode + '_noise_color_' + noise_color + '.png')
        plt.show()
